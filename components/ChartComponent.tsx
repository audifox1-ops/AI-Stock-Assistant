    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white border border-gray-100 p-10 rounded-[3.5rem] shadow-[0_25px_70px_rgba(0,0,0,0.08)] min-w-fit overflow-visible">
          ...
          <p className="text-2xl font-black text-[#191f28] leading-none whitespace-nowrap px-1 overflow-visible">{payload[0].value.toLocaleString()}원</p>
        </div>
      );
    }
    return null;
      ...
      <header className="px-8 pt-16 pb-10 overflow-visible">
        ...
        <div className="flex gap-12 border-b-[2.5px] border-gray-50 overflow-visible px-2">
          <button 
            ...
            className={`pb-6 px-4 text-lg font-black transition-all relative min-w-fit \${activeTab === 'holdings' ? 'text-[#3182f6]' : 'text-gray-300 hover:text-gray-500'}`}
          >
            내 자산 리스트
          ...
          <button 
            ...
            className={`pb-6 px-4 text-lg font-black transition-all relative min-w-fit \${activeTab === 'interests' ? 'text-[#3182f6]' : 'text-gray-300 hover:text-gray-500'}`}
          >
            관심 종목
      ...
      {selectedStock && (
        <div className="px-8 space-y-16 animate-in slide-in-from-bottom-8 duration-800 overflow-visible">
            ...
            <div className="flex justify-between items-start mb-16 overflow-visible px-10">
              <div className="overflow-visible min-w-fit">
                ...
                <div className="flex items-center gap-6 overflow-visible">
                  <span className="text-sm font-black text-gray-300 uppercase tracking-[0.25em] ml-1 overflow-visible">{selectedStock.symbol}</span>
                  <div className={`flex items-center gap-3 px-8 py-3 rounded-full font-black text-xs shadow-sm border min-w-fit overflow-visible \${isTrendUp ? 'bg-red-50 text-red-500 border-red-100' : 'bg-blue-50 text-[#3182f6] border-blue-100'}`}>
                    {isTrendUp ? <TrendingUp size={18} strokeWidth={3} /> : <TrendingDown size={18} strokeWidth={3} />}
                    <span className="mb-0.5 whitespace-nowrap overflow-visible">{isTrendUp ? '우상향 시그널' : '바닥 다지기 중'}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="h-[420px] w-full px-6 overflow-visible">
              {isDataLoading ? (
                <div className="w-full h-full flex flex-col items-center justify-center gap-8">
                  ...
                  <p className="text-base font-black text-gray-200 animate-pulse uppercase tracking-[0.4em] overflow-visible">Visualizing Market Data</p>
                </div>
              ) : (
          ...
          <div className="bg-gray-50 border border-gray-100 rounded-[3.5rem] p-12 space-y-10 shadow-sm overflow-visible relative group">
            <div className="absolute -top-6 left-12 px-10 py-3.5 bg-[#3182f6] text-white rounded-full font-black text-sm shadow-xl shadow-blue-100 flex items-center gap-3 z-10">
              <Sparkles size={18} />
              AI Insight Pro
            </div>
            <p className="text-lg text-gray-600 leading-relaxed font-bold break-keep whitespace-normal px-1 mt-4 overflow-visible">
              {isTrendUp 
